...
func NewSmsMobile(service *message.Service) *sms {
	return &sms{service: service}
}

func (h *sms) Register(c echo.Context) error {
	return h.handle(c, message.TypeEmailRegister)
}

func (h *sms) Funding(c echo.Context) error {
	return h.handle(c, message.TypeEmailFunding)
}
...
func (h *sms) handle(c echo.Context, smsType string) error {
	...
	msg := message.NewMessage(smsType, "", h.To, h.Params)
	if !msg.IsValidType() {
		return c.JSON(http.StatusOK, map[string]interface{}{
